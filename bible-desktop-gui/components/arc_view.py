...
class ArcView(QWidget):
    ...
    def render(self):
        ...
        for conn in connections:
            ...
            # Draw circular arc using Pythagorean theorem (matches web visualizer)
            # Formula: y = sqrt(r² - x²) for perfect circular arcs
            radius = distance / 2
            center = (x1 + x2) / 2
            ...
            for x_pos in x_points:
                ...
                # Apply Pythagorean theorem: y = sqrt(r² - x²)
                if abs(center_offset) <= radius:
                    y_offset = np.sqrt(radius**2 - center_offset**2)
                    # Scale height for visibility
                    y_points.append(y_offset * height / radius if radius > 0 else 0)
                else:
                    y_points.append(0)

            ax.plot(x_points, y_points, color=color, linewidth=width, alpha=0.3)
