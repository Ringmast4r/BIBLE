...
from components.chord_view import ChordView
from components.sunburst_view import SunburstView
...
# Import theographic data loader
...
class BibleVisualizerApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.chord_view = ChordView()
        self.sunburst_view = SunburstView()
        self.geographic_map_view = GeographicMapView()
        self.timeline_view = TimelineView()
        self.people_network_view = PeopleNetworkView()
        self.stats_view = StatsView()

        self.tabs.addTab(self.network_view, '3D Network Graph')
        self.tabs.addTab(self.arc_view, 'Arc Diagram')
        self.tabs.addTab(self.chord_view, 'Chord Diagram')
        self.tabs.addTab(self.heatmap_view, 'Heatmap')
        self.tabs.addTab(self.sunburst_view, 'Sunburst')
        self.tabs.addTab(self.geographic_map_view, 'Geographic Map')
        self.tabs.addTab(self.timeline_view, 'Timeline')
        self.tabs.addTab(self.people_network_view, 'People Network')
    ...
    def load_data(self):
        """Load Bible cross-reference data and theographic data"""
        try:
            # Load graph data
            ...
            # Pass data to cross-reference views
            print("Loading cross-reference visualizations...")
            print("  - Network View")
            ...
            print("  - Chord View")
            self.chord_view.set_data(self.data)
            print("  - Sunburst View")
            self.sunburst_view.set_data(self.data)
            print("  - Stats View")
            self.stats_view.set_data(self.stats)

            # Load theographic data
            print("\nLoading theographic data...")
            self.theographic_loader = TheographicDataLoader()
            if self.theographic_loader.load():
                # Pass theographic data to relevant views
                print("\nLoading theographic visualizations...")
                print("  - Geographic Map View")
                self.geographic_map_view.set_theographic_data(self.theographic_loader)
                print("  - Timeline View")
                self.timeline_view.set_theographic_data(self.theographic_loader)
                print("  - People Network View")
                self.people_network_view.set_theographic_data(self.theographic_loader)

                self.status_bar.showMessage(
                    f'Loaded {len(self.data["connections"])} connections, '
                    f'{len(self.theographic_loader.get_people())} people, '
                    f'{len(self.theographic_loader.get_places())} places'
                )
            else:
                self.status_bar.showMessage(
                    f'Loaded {len(self.data["connections"])} connections '
                    f'(Theographic data unavailable)'
                )
